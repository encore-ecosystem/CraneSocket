...
async fn test_text_data_transfer_success() {
    ...
    let receiver_handle = tokio::spawn(async move {
        ...
        timed(ws_stream.send(&ClientMessage::text(text.clone()).as_bytes()))
            .await
...
async fn test_text_data_transfer_alone_in_room() {
    ...
    let res = timed(ws_stream.send(&ClientMessage::text(test_message.clone()).as_bytes())).await;
    assert!(res.is_ok());
...
async fn test_binary_data_transfer_success() {
    ...
    let receiver_handle = tokio::spawn(async move {
        ...
        timed(ws_stream.send(&ClientMessage::binary(binary_message).as_bytes()))
            .await
            .expect("Failed to send message");
    });
