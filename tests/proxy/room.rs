...
async fn test_create_room() {
    ...
    let (mut first_conn, _) = WebSocketConnection::create_room(&server_addr)
        .await
        .unwrap();
    ...
    let (mut second_conn, _) = WebSocketConnection::create_room(&server_addr)
        .await
        .unwrap();
    ...
    first_conn.close(None).await.unwrap();
    tokio::time::sleep(tokio::time::Duration::from_millis(TEST_SLEEP_TIME_MS)).await;

    assert_eq!(rooms.read().await.len(), 1);
    assert_eq!(peer2room.read().await.len(), 1);

    second_conn.close(None).await.unwrap();
    tokio::time::sleep(tokio::time::Duration::from_millis(TEST_SLEEP_TIME_MS)).await;
...
async fn test_create_and_join_room() {
    ...
    let (mut create_room_conn, room_id) = WebSocketConnection::create_room(&server_addr)
        .await
        .unwrap();
    ...
    let mut join_room_conn = WebSocketConnection::join_room(&server_addr, room_id)
        .await
        .unwrap();
    ...
    let msg = create_room_conn.next().await.unwrap();

    assert_eq!(msg, ServerMessage::ClientJoined);

    join_room_conn.close(None).await.unwrap();
    tokio::time::sleep(tokio::time::Duration::from_millis(TEST_SLEEP_TIME_MS)).await;

    assert_eq!(rooms.read().await.len(), 1);
    assert_eq!(peer2room.read().await.len(), 1);

    create_room_conn.close(None).await.unwrap();
    tokio::time::sleep(tokio::time::Duration::from_millis(TEST_SLEEP_TIME_MS)).await;
...
#[tokio::test]
async fn test_join_room_failure() {
    let bind_addr = "127.0.0.1:0".parse::<SocketAddr>().unwrap();
    ...
    let res = WebSocketConnection::join_room(&server_addr, room_id)
        .await
        .is_err();
